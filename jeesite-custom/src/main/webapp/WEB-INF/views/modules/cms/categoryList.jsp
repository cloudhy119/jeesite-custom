		...
		<table id="treeTable" class="table table-striped table-bordered table-condensed">
			...
			<c:forEach items="${list}" var="tpl">
				<tr id="${tpl.id}" pId="${tpl.parent.id ne '1'?tpl.parent.id:'0'}">
					...
					<td>
						<a class="btn-operate" href="${pageContext.request.contextPath}${fns:getFrontPath()}/list-${tpl.id}${fns:getUrlSuffix()}" target="_blank">访问</a>
						<shiro:hasPermission name="cms:category:edit">
							<a class="btn-operate" href="${ctx}/cms/category/form?id=${tpl.id}">修改</a>
							<a class="btn-operate" href="${ctx}/cms/category/delete?id=${tpl.id}" onclick="return confirmx('要删除该栏目及所有子栏目项吗？', this.href)">删除</a>
							<a class="btn-operate" href="${ctx}/cms/category/form?parent.id=${tpl.id}">添加下级栏目</a>
						</shiro:hasPermission>
					</td>
				</tr>
			</c:forEach>
