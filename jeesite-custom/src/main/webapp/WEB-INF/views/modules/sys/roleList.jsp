		...
		<c:forEach items="${list}" var="role">
			<tr>
				...
				<shiro:hasPermission name="sys:role:edit"><td>
					<a class="btn-operate" href="${ctx}/sys/role/assign?id=${role.id}">分配</a>
					<c:if test="${(role.sysData eq fns:getDictValue('是', 'yes_no', '1') && fns:getUser().admin)||!(role.sysData eq fns:getDictValue('是', 'yes_no', '1'))}">
						<a class="btn-operate" href="${ctx}/sys/role/form?id=${role.id}">修改</a>
					</c:if>
					<a class="btn-operate" href="${ctx}/sys/role/delete?id=${role.id}" onclick="return confirmx('确认要删除该角色吗？', this.href)">删除</a>
