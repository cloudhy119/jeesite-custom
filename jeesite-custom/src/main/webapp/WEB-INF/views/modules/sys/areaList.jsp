	...
	<script type="text/template" id="treeTableTpl">
		<tr id="{{row.id}}" pId="{{pid}}">
			...
			<shiro:hasPermission name="sys:area:edit"><td>
				<a class="btn-operate" href="${ctx}/sys/area/form?id={{row.id}}">修改</a>
				<a class="btn-operate" href="${ctx}/sys/area/delete?id={{row.id}}" onclick="return confirmx('要删除该区域及所有子区域项吗？', this.href)">删除</a>
				<a class="btn-operate" href="${ctx}/sys/area/form?parent.id={{row.id}}">添加下级区域</a> 
