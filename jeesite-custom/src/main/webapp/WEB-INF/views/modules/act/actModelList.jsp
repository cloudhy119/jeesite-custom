		...
		<tbody>
			<c:forEach items="${page.list}" var="model">
				<tr>
					...
					<td>
						<a class="btn-operate" href="${pageContext.request.contextPath}/act/process-editor/modeler.jsp?modelId=${model.id}" target="_blank">编辑</a>
						<a class="btn-operate" href="${ctx}/act/model/deploy?id=${model.id}" onclick="return confirmx('确认要部署该模型吗？', this.href)">部署</a>
						<a class="btn-operate" href="${ctx}/act/model/export?id=${model.id}" target="_blank">导出</a>
	                    <a class="btn-operate" href="${ctx}/act/model/delete?id=${model.id}" onclick="return confirmx('确认要删除该模型吗？', this.href)">删除</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
