	...
	<script type="text/template" id="treeTableTpl">
		<tr id="{{row.id}}" pId="{{pid}}">
			...
			<shiro:hasPermission name="test:testTree:edit"><td>
   				<a class="btn-operate" href="${ctx}/test/testTree/form?id={{row.id}}">修改</a>
				<a class="btn-operate" href="${ctx}/test/testTree/delete?id={{row.id}}" onclick="return confirmx('确认要删除该树结构及所有子树结构吗？', this.href)">删除</a>
				<a class="btn-operate" href="${ctx}/test/testTree/form?parent.id={{row.id}}">添加下级树结构</a> 
