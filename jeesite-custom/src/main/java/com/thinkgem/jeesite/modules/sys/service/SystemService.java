...
public class SystemService extends BaseService implements InitializingBean {
	...
	public List<String> findAccListByRoleEnName(User userQuery) {
		List<String> auditUserList = new ArrayList<String>();
		List<User> userList = findListByRoleEnName(userQuery);
		if(userList != null && !userList.isEmpty()) {
			for(User user : userList) {
				auditUserList.add(user.getLoginName());
			}
		}
		return auditUserList;
	}
	public List<User> findListByRoleEnName(User user) {
		return userDao.findListByRoleEnName(user);
	}
	/**
	 * 根据登录名获取用户
	 * @param loginName
	...
	public void saveUser(User user) {
		user.setStatus("1"); //通过后台添加的用户默认都是审核通过状态
		user.setUserType("1"); //通过后台添加的用户默认都是系统管理员（区别于前端用户）
