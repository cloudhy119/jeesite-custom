...
public class CmsUtils {
	 ...
	 * ${fnc:getArticleList(category.site.id, category.id, not empty pageSize?pageSize:8, 'posid:2, orderBy: \"hits desc\"')}"
	 */
	public static List<Article> getArticleList2(String siteId, String categoryId, int number,int endnumber, String param){
		Page<Article> page = new Page<Article>(number, endnumber, -1);
		Category category = new Category(categoryId, new Site(siteId));
		category.setParentIds(categoryId);
		Article article = new Article(category);
		if (StringUtils.isNotBlank(param)){
     ...
     * @return url
     */
    public static String getUrlDynamic2(Article article) {
    	if(StringUtils.isNotBlank(article.getLink())){
    		return article.getLink();
    	}
    	StringBuilder str = new StringBuilder();
    	...
    	str.append("/news");
    	str.append("/view-").append(article.getCategory().getId()).append("-").append(article.getId()).append(Global.getUrlSuffix());
//    	str.append("?headType=").append(article.getHeadType());
//    	str.append("&countryCode=").append(article.getCountryCode());
    	return str.toString();
    }
