...
//		dc.createAlias("category.site", "category.site");
		if (article.getCategory()!=null && StringUtils.isNotBlank(article.getCategory().getId()) && !Category.isRoot(article.getCategory().getId())){
			...
			if(StringUtils.isNotBlank(article.getCategory().getKeywords())){
				category.setKeywords(article.getCategory().getKeywords());
			}
...
//		return dao.find(page, dc);
	...
	public void save(Article article) {
		...
		if(article.getArticleData().getFile() != null && !StringUtils.isBlank(article.getArticleData().getFile().getFilePath())){
			if (StringUtils.isBlank(article.getArticleData().getFile().getId())){
				SysFile file = article.getArticleData().getFile();
				file.setItemId(article.getId());
				file.setFileName(file.getFilePath().substring(file.getFilePath().lastIndexOf("/")+1, file.getFilePath().lastIndexOf(".")));
				file.setItemCode("CMS");
				file.preInsert();
				sysFileDao.insert(file);
			}else{
				SysFile file = article.getArticleData().getFile();
				file.setItemId(article.getId());
				file.setFileName(file.getFilePath().substring(file.getFilePath().lastIndexOf("/")+1, file.getFilePath().lastIndexOf(".")));
				file.setItemCode("CMS");
				file.preUpdate();
				sysFileDao.update(file);
			}
		}
	}
	public static void main(String[] args) {
		String file="/asean/userfiles/1/files/cms/article/2017/12/bannner.rar";
		String str=file.substring(file.lastIndexOf("/")+1,file.lastIndexOf("."));
